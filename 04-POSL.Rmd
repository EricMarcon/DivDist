...
Accum <- DivAccum(POSL_VA_wmppp, 
                  r.seq=c(seq(from=0, to=50, by=5), seq(from=100, to=500, by=50)), 
                  q.seq = 0:2, 
                  H0 = "None",
                  Individual = TRUE)
```
...
```{r}
par(mar=c(0,0,0,0))
MapPlot(Accum, Order=0, NeighborHood=25)
# Arbres abattus
BDD_Vivants_et_Abattus_renseignes_POSL %>% 
  filter(Etat == "Abattu") -> 
  Abattus
...
```{r}
par(mar=c(0,0,0,0))
MapPlot(Accum, Order=2, NeighborHood=25)
# Arbres abattus
BDD_Vivants_et_Abattus_renseignes_POSL %>% 
  filter(Etat == "Abattu") -> 
  Abattus
